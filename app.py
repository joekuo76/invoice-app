...
st.info("🔒 V4 不使用 Gemini API，也不需要 API Key；QR 使用 ZXing-C++，OCR 使用 EasyOCR。")


TEMPLATE_PATH = "template.xlsx"
CACHE_DB = "receipt_cache_local.sqlite3"
OCR_ENGINE_VERSION = "easyocr-zxing-v4"
MAX_IMAGE_SIDE = 2600
...
with st.sidebar:
    ...
    st.divider()
    st.write("**辨識方式**")
    st.code("ZXing-C++ + EasyOCR", language=None)
    st.caption("沒有每日 API 額度限制；較模糊、皺摺或特殊版型單據仍建議人工確認。")

    if st.button("🗑️ 清除本機辨識快取", width="stretch"):
        try:
...
@st.cache_resource(show_spinner="第一次載入 OCR 模型，可能需要幾分鐘…")
def get_ocr_engine():
    if easyocr is None:
        raise RuntimeError("EasyOCR 尚未安裝，請確認 requirements.txt。")
    # 繁體中文 + 英文；Streamlit Cloud 使用 CPU 模式
    return easyocr.Reader(["ch_tra", "en"], gpu=False, verbose=False)


def run_ocr(img: Image.Image) -> tuple[list[str], list[float]]:
    engine = get_ocr_engine()
    result = engine.readtext(np.array(img), detail=1, paragraph=False)
...
def parse_local_receipt(image_bytes: bytes) -> dict:
    ...
    qr_texts = decode_qr_codes(pil_img)
...
if uploaded_files:
    if st.button("🚀 開始本機辨識", type="primary", width="stretch"):
        parsed_results = []
        st.session_state["cache_hits_local"] = 0
...
with st.expander("ℹ️ 無 Gemini 版辨識方式"):
    st.markdown(
        ...
        **辨識流程**
        ...
        1. 先使用 ZXing-C++ 嘗試讀取台灣電子發票 QR Code。
        2. 再使用 EasyOCR 做繁體中文 / 英數字 OCR。
...
- 不需要 `packages.txt`。
- 不需要 `libgl1`。
- 建議 Streamlit Cloud 使用 Python 3.12。
- 第一次辨識可能需要下載 OCR 模型。
